        ...
        <?php else: ?>
            ...
            <div class="med-card">
                ...
                <div class="med-card-body">
                    <div class="med-card-title"><?= h($med['name']) ?></div>
                    <div class="med-card-vendor">&#127981; <?= h($med['vendor_name']) ?></div>
    ...
    function renderGrid(meds) {
        ...
        meds.forEach(function(m) {
            ...
            var stockHtml = parseInt(m.availability) > 0
                ? '&#9989; ' + esc(m.availability) + ' in stock'
                : '<span class="out-of-stock">&#10060; Out of stock</span>';
            var cartHtml = '';
            <?php if (isCustomer()): ?>
            if (parseInt(m.availability) > 0) {
                cartHtml =
                    '<input type="number" class="qty-input" min="1" max="' + esc(m.availability) + '" value="1" id="qty_ajax_' + m.id + '">' +
            ...
            html += '<div class="med-card">' +
                ...
                '<div class="med-card-body">' +
                    '<div class="med-card-title">' + esc(m.name) + '</div>' +
                    '<div class="med-card-vendor">&#127981; ' + esc(m.vendor_name) + '</div>' +
                    '<span class="med-card-type type-' + esc(m.category_type) + '">' + (m.category_type === 'liquid' ? 'Liquid' : 'Solid') + '</span>' +
