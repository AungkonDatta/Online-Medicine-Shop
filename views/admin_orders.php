...
(function () {
    ...
    function statusBadge(status) {
        return '<span class="status-' + esc(status) + '">' + status.charAt(0).toUpperCase() + status.slice(1) + '</span>';
    }
    ...
    function render(rows) {
        ...
        rows.forEach(function (o) {
            ...
            if (o.status === 'pending') {
                actionHtml +=
                    '<a class="btn-sm btn-accept" href="index.php?page=admin_orders&action=accept&id=' + o.id +
                    '" onclick="return confirm(\'Accept order #' + o.id + '?\')">Accept</a>' +
                    '<a class="btn-sm btn-reject" href="index.php?page=admin_orders&action=reject&id=' + o.id +
                    '" onclick="return confirm(\'Reject order #' + o.id + '?\')">Reject</a>';
            }
